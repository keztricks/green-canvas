...
document.getElementById('addressSearch').addEventListener('input', function(e) {
    ...
    if (searchTerm === '') {
        // Show all addresses if search is empty
    ...
    // Split search terms by spaces to allow multiple search criteria
    const searchTerms = searchTerm.split(/\s+/);
    
    addresses.forEach(address => {
        const street = address.dataset.street;
        const house = address.dataset.house;
        const fullAddress = `${house} ${street}`; // Combined for matching
        
        // Check if ALL search terms match somewhere in the address
        const allTermsMatch = searchTerms.every(term => {
            return street.includes(term) || house.includes(term);
        });
