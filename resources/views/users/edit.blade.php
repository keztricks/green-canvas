    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <h2 class="text-3xl font-bold text-gray-800 dark:text-white mb-6">Edit User</h2>
                ...
                <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-6">
                    ...
                    <div>
                        <label for="role" class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Role</label>
                        <select name="role" id="role" required
                                class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#6AB023] @error('role') border-red-500 @enderror"
                        ...
                        @if(auth()->user()->isWardAdmin())
                            ...
                            <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Ward admins cannot change user roles</p>
                        @endif
                        @error('role')
                            <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div id="wardsSection" class="{{ in_array($user->role, ['ward_admin', 'canvasser']) ? '' : 'hidden' }}">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Assigned Wards</label>
                        <p class="text-xs text-gray-600 dark:text-gray-400 mb-2">Select the wards this user can access (Ward Admins and Canvassers only)</p>
                        <div class="border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded p-3 max-h-60 overflow-y-auto space-y-2">
                            @foreach($wards as $ward)
                                <label class="flex items-center space-x-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-600 p-2 rounded">
                                    ...
                                    <span class="text-sm text-gray-700 dark:text-gray-200">{{ $ward->name }}</span>
                                </label>
                            @endforeach
                        </div>
                        @error('wards')
                    ...
                    <div id="exportSchedulesSection" class="{{ in_array($user->role, ['ward_admin', 'canvasser']) ? '' : 'hidden' }} border-t border-gray-200 dark:border-gray-700 pt-6">
                        <p class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Export Email Schedule</p>
                        <p class="text-xs text-gray-600 dark:text-gray-400 mb-4">Configure automatic export email frequency for each assigned ward</p>
                        ...
                        <div id="exportSchedulesList" class="space-y-4">
                            @foreach($wards as $ward)
                                <div class="export-schedule-ward {{ in_array($ward->id, old('wards', $user->wards->pluck('id')->toArray())) ? '' : 'hidden' }}" data-ward-id="{{ $ward->id }}">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">{{ $ward->name }}</label>
                                    <div class="flex gap-4">
                                        ...
                                        <label class="flex items-center cursor-pointer">
                                            ...
                                            <span class="schedule-option px-4 py-2 border border-gray-300 dark:border-gray-600 rounded {{ $currentFrequency === 'none' ? 'bg-[#6AB023] text-white border-[#6AB023]' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600' }}">None</span>
                                        </label>
                                        ...
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="export_schedules[{{ $ward->id }}]" value="daily" 
                                                   ...
                                                   class="sr-only ward-schedule-radio">
                                            <span class="schedule-option px-4 py-2 border border-gray-300 dark:border-gray-600 rounded {{ $currentFrequency === 'daily' ? 'bg-[#6AB023] text-white border-[#6AB023]' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600' }}">Daily</span>
                                        </label>
                                        ...
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="export_schedules[{{ $ward->id }}]" value="weekly" 
                                                   ...
                                                   class="sr-only ward-schedule-radio">
                                            <span class="schedule-option px-4 py-2 border border-gray-300 dark:border-gray-600 rounded {{ $currentFrequency === 'weekly' ? 'bg-[#6AB023] text-white border-[#6AB023]' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600' }}">Weekly</span>
                                        </label>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                        <p class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Change Password (optional)</p>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Leave blank to keep current password. Minimum 8 characters, mixed case, and numbers required</p>
                                @error('password')
                                    <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Confirm New Password</label>
                    ...
                    <div class="flex gap-3 pt-4">
                        ...
                        <a href="{{ route('users.index') }}" class="bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-800 dark:text-white px-6 py-2 rounded font-medium">
                            Cancel
                        </a>
                    </div>
                </form>
